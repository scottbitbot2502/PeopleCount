...
void disableBootButtonReset() {
    // Disable automatic reset on GPIO0
    REG_WRITE(RTC_CNTL_BROWN_OUT_REG, 0);  // Disable brownout detector
    REG_WRITE(RTC_CNTL_WDTCONFIG0_REG, 0); // Disable hardware watchdog
    REG_WRITE(RTC_CNTL_WDTWPROTECT_REG, 0); // Disable watchdog write protection
    
    // Configure GPIO0 as a normal input
    rtc_gpio_deinit(GPIO_NUM_0);
    gpio_reset_pin(GPIO_NUM_0);
    gpio_set_direction(GPIO_NUM_0, GPIO_MODE_INPUT);
    gpio_set_pull_mode(GPIO_NUM_0, GPIO_PULLUP_ONLY);
...
void start_boot_menu(void) {
    ...
    // Disable boot button reset functionality
    disableBootButtonReset();
    ...
    // Ensure the watchdog timer is reset during normal operation
    ...
    while (WiFi.status() == WL_DISCONNECTED && attempts < maxAttempts) {
