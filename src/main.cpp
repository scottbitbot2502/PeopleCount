...
void setup() {
  ...
  initWiFiSetup();
  while (isWiFiSetupInProgress) {
      delay(10); // Small delay to prevent busy-waiting
  }
  ESP_LOGI(TAG, "WiFi setup completed.");
  // Handle WiFi setup requests
  // Initialize other modules that depend on WiFi here
  // Uncomment the following lines if these modules require WiFi
  // sensor_init(); // Initialize sensors that require WiFi
  // mqtt_init(); // Initialize MQTT client that requires WiFi
  // etc.

  // Disable brownout detector
  *((uint32_t *)ETS_UNCACHED_ADDR((DR_REG_RTCCNTL_BASE + 0xd4))) = 0;
...
void loop() {
  // Call the WiFi setup handling function if in setup mode
